...
installChaincode() {
  ...
  VERSION=${3:-1.0}
  set -x
  peer chaincode install -n recordcontract -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt
  res=$?
  set +x
  cat log.txt
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n recordcontract -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.prontuchain.MedicalRecord:instantiate"]}' -P "OR ('Org1MSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n recordcontract -l ${LANGUAGE} -v 1.0 -c '{"Args":["org.prontuchain.MedicalRecord:instantiate"]}' -P "OR ('Org1MSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
